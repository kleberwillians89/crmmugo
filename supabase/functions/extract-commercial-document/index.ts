...
const extractionSchema={type:'object',additionalProperties:false,required:['documentType','confidence','warnings','missingFields','conflicts','extractedData'],properties:{documentType:{type:'string',enum:['proposal','signed_contract','unsigned_contract','amendment','other']},confidence:{type:'number',minimum:0,maximum:1},warnings:{type:'array',items:{type:'string'}},missingFields:{type:'array',items:{type:'string'}},conflicts:{type:'array',items:{type:'string'}},extractedData:{type:'object',additionalProperties:false,required:['client','proposal','contract','financial','services','clauses'],properties:{client:clientSchema,proposal:proposalSchema,contract:contractSchema,financial:financialSchema,services:{type:'array',items:serviceSchema},clauses:clausesSchema}}}}
...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    if(analysis.mime_type==='application/pdf'&&rawText.length<80){
      const result={documentType:'other',confidence:0,warnings:['O documento parece ser uma imagem digitalizada. A extração automática pode ser limitada.'],missingFields:['Conteúdo textual pesquisável'],conflicts:[],extractedData:null}
    ...
    const sanitized={...extracted,warnings:(extracted.warnings||[]).map((item:unknown)=>String(item).slice(0,300)),missingFields:(extracted.missingFields||[]).map((item:unknown)=>String(item).slice(0,120)),conflicts:(extracted.conflicts||[]).map((item:unknown)=>String(item).slice(0,300))}
