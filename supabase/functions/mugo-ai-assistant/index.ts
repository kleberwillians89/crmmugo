...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    const [clientsResult,proposalsResult,contractsResult,installmentsResult]=await Promise.all([
      ...
      client.from('contracts').select('id,client_id,proposal_id,responsible_id,status,signed,start_date,end_date,setup_value,setup_received_amount,monthly_value,total_value,team_members(name),contract_services(service_name)').eq('organization_id',org).limit(1000),
