...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    const [clientsResult,proposalsResult,contractsResult,installmentsResult]=await Promise.all([
      ...
      client.from('proposals').select('id,client_id,status,sent_at,closed_at,lost_at,total_value,setup_value,monthly_value,responsible,proposal_services(service_name)').eq('organization_id',org).limit(1000),
      client.from('contracts').select('id,client_id,proposal_id,status,signed,start_date,end_date,setup_value,setup_received_amount,monthly_value,total_value,contract_services(service_name)').eq('organization_id',org).limit(1000),
    ...
    const groupRevenue=(field:'service'|'responsible')=>{const map=new Map<string,number>();activeContracts.forEach((contract:any)=>{const labels=field==='service'?(contract.contract_services?.map((service:any)=>service.service_name)||['Não informado']):[proposals.find((proposal:any)=>proposal.id===contract.proposal_id)?.responsible||'Sem responsável'];labels.forEach((label:string)=>map.set(label,(map.get(label)||0)+money(contract.monthly_value)))});return [...map].map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value).slice(0,15)}
    ...
    const openaiResponse=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal:AbortSignal.timeout(15000),headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model,instructions:systemPrompt,input,max_output_tokens:700,store:false})})
