...
const pulseRepository=readFileSync(new URL('../src/services/data/pulseRepository.js',import.meta.url),'utf8')
assert.match(pulseRepository,/rpc\('sync_pulse_alerts',\{detected_alerts:alerts\}\)/,'sync RPC must send only detected_alerts')
assert.doesNotMatch(pulseRepository,/execution_id|execution_scope|organization_id|source|user_id/,'sync RPC must not send extra parameters')
assert.match(pulseRepository,/pulseSyncBlocked=true/,'inactive users must block subsequent sync attempts')
