...
assert.match(page, /conversationsRef\.current\.find\(item=>conversationKey\(item\)===selectedId\)/)
assert.doesNotMatch(page, /loadHistory\(selectedId\)/)
assert.equal((page.match(/timer=setTimeout\(poll,30000\)/g)||[]).length,2)
assert.match(page, /realtimeConnectedRef\.current\?60000:30000/)
...
assert.doesNotMatch(repository.slice(repository.indexOf('export async function listConversations'), repository.indexOf('export async function findConversationByPhone')), /invoke\('list_(conversations|messages)'/)
assert.match(repository, /provider_message_id/)
assert.match(repository, /raw_payload/)
assert.match(edge, /IDEMPOTENCY_KEY_MISSING/)
assert.match(edge, /MESSAGE_SEND_UNCONFIRMED/)
assert.match(authGuard, /Authorization/)
assert.match(app, /pulseSyncUnavailable\.current\s*=\s*true/)
assert.match(app, /sessionStorage\.setItem\(\s*["']mugo:pulse-sync-unavailable["']/)
