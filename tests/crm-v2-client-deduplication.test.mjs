...
import {
  ...
  normalizePhone,
  normalizeWebsite,
} from "../src/lib/clientDeduplication.js";
...
assert.ok(nameSimilarity("Santo Circuito", "Santo Circuto") > 0.78);
const organization = "00000000-0000-0000-0000-000000000001";
const clients = [
  {
    id: "a",
    organization_id: organization,
    company_name: "Amalie LTDA",
    document_number: "123",
    phone: "11999990000",
  },
  {
  ...
  { id: "c", organization_id: organization, company_name: "Outro cliente" },
];
assert.equal(compareClients(clients[0], clients[1]).level, "strong");
assert.equal(
  compareClients(clients[0], { ...clients[1], organization_id: "other" }).level,
  "blocked",
);
...
const page = fs.readFileSync(
  new URL("../src/components/ClientsPage.jsx", import.meta.url),
  "utf8",
);
assert.ok(
  page.indexOf("client-list-primary") < page.indexOf("client-form-drawer"),
  "A lista precisa aparecer antes do formulário",
);
...
for (const token of [
  ...
  assert.ok(migration.includes(token), `Garantia ausente: ${token}`);
assert.doesNotMatch(
  migration,
  /delete from public\.(clients|contracts|proposals|invoice_installments|payments|documents)/i,
);
...
for (const client of [
  ...
  assert.ok(audit.includes(client));
