...
import {
  ...
  createProposal,
  updateProposal,
} from "./services/data/proposalsRepository";
...
import {
  listPulseAlerts,
  syncPulseAlerts as rawSyncPulseAlerts,
} from "./services/data/pulseRepository";
...
import {
  AccountsPayablePage,
  CashFlowPage,
  ClientContractReviewPage,
  FinancialReportsPage,
  RecurringAccountsPage,
} from "./components/FinanceV2Pages";
...
export default function App() {
  ...
  useEffect(() => {
    const restore = () => setActivePage(pageFromLocation());
    window.addEventListener("popstate", restore);
    return () => window.removeEventListener("popstate", restore);
  }, []);
  useEffect(() => {
    const navigate = (event) => handleNavigate(event.detail);
    window.addEventListener("mugo:navigate", navigate);
    return () => window.removeEventListener("mugo:navigate", navigate);
  });
  useEffect(() => {
    const refresh = () => loadProposals();
    window.addEventListener(CRM_DATA_CHANGED, refresh);
    return () => window.removeEventListener(CRM_DATA_CHANGED, refresh);
  }, []);
      ...
      try {
        const [data, contractRows] =
          dataProvider === "supabase"
            ? await Promise.all([getProposals(), listContracts()])
            : [await getProposals(), []];
        setProposals(data);
        if (dataProvider === "supabase") {
          ...
          try {
            ...
            if (import.meta.env.DEV)
              console.error("[CRM] Partial analytics load failed", {
            ...
            setIntelligenceError(
              "Parte dos dados analíticos não pôde ser carregada. Os resultados exibidos consideram somente os registros disponíveis.",
            );
          }
        }
      } catch (error) {
        if (import.meta.env.DEV)
          console.error("[CRM] Initial load failed", {
            message: error?.message,
            code: error?.code,
            details: error?.details,
            hint: error?.hint,
        ...
        setErrorMessage(
          [
            ...
            .filter(Boolean)
            .join(" · ") || "Não foi possível carregar as propostas.",
        );
      } finally {
        setLoading(false);
        setHasLoaded(true);
  ...
  function handleNavigate(page) {
    ...
    if (page === "dashboard" || page.startsWith("intelligence-"))
      loadProposals();
    setMessage("");
    setErrorMessage("");
  }
  ...
  async function handleDocumentImported(imported) {
    ...
    await loadProposals();
    setActivePage(
      imported.documentType === "proposal"
        ? "proposals"
        : imported.documentType === "other"
          ? "clients"
          : "contracts",
    );
  }
  ...
  function validateForm() {
    if (dataProvider === "supabase" && !form.client_id)
      return "Selecione um cliente existente.";
    if (dataProvider !== "supabase" && !form.client_name.trim())
    ...
    if (!form.setup_value && !form.monthly_value)
      return "Valor de implantação ou valor mensal obrigatórios.";
    return "";
  }
    ...
    try {
      ...
      setErrorMessage(
        [
          ...
          .filter(Boolean)
          .join(" · ") ||
          "Erro ao salvar proposta. Verifique os dados e tente novamente.",
      );
    } finally {
  ...
  function handleEdit(proposal) {
    setForm({
      ...
      proposal_sent_date: buildDateValue(
        proposal.sentAt || proposal.proposal_sent_date,
      ),
      responsible_id: proposal.responsibleId || "",
      proposal_status: proposal.status || proposal.proposal_status || "",
      contract_signed:
        proposal.hasContract ?? proposal.contract_signed ?? false,
      contract_term: proposal.contractTermMonths
        ? `${proposal.contractTermMonths} meses`
        : proposal.contract_term || "Sem contrato",
      ...
      contracts:
        dataProvider === "supabase"
          ...
          : proposals.map((proposal) => ({
              ...proposal,
              status: proposal.contract_signed
                ? "active"
                : proposal.proposal_status,
              signed: Boolean(proposal.contract_signed),
              start_date: proposal.contract_start_date,
              end_date: proposal.contract_end_date,
              contract_services: proposal.main_service
                ? [{ service_name: proposal.main_service }]
                : [],
              clients: {
                company_name: proposal.company || proposal.client_name,
              },
            })),
      clients,
      installments,
  ...
  useEffect(() => {
    if (!hasLoaded || dataProvider !== "supabase") return undefined;
    let mounted = true;
    const canSynchronize = ["admin", "manager"].includes(profile?.role);
    const monitor = async () => {
      const syncPulseAlerts = pulseSyncUnavailable.current
          ? async () => {}
          : rawSyncPulseAlerts,
        generated = generatePulseAlerts({
          proposals,
      ...
      try {
        if (canSynchronize) await syncPulseAlerts(generated);
        const persisted = await listPulseAlerts({ status: "all" });
        if (mounted) setPulseAlerts(persisted);
      } catch (error) {
        const missingRpc =
          error?.code === "PGRST202" ||
          error?.status === 404 ||
          /sync_pulse_alerts|schema cache/i.test(
            `${error?.message || ""} ${error?.details || ""}`,
          );
        if (missingRpc) {
          pulseSyncUnavailable.current = true;
          sessionStorage.setItem("mugo:pulse-sync-unavailable", "1");
        }
        if (mounted && canSynchronize)
          setPulseAlerts(
            generated.map((alert) => ({
              ...alert,
    ...
    monitor();
    const timer = canSynchronize ? setInterval(monitor, 300000) : null;
    return () => {
      mounted = false;
  ...
  async function refreshPulse() {
    try {
      setPulseAlerts(await listPulseAlerts({ status: "all" }));
    } catch {
      return;
    }
  }
      ...
      >
        {import.meta.env.VITE_WHATSAPP_DEMO_MODE === "true" && (
          <div className="demo-mode-banner" role="status">
            Modo demonstração: envios e automações estão desativados.
          </div>
        )}
        <PulseBell
          alerts={pulseAlerts}
          onOpen={() => handleNavigate("intelligence-attention")}
        />
        ...
        <div className="content-container">
          {activePage !== "whatsapp" && (
            <GlobalSearch
              ...
              onOpen={(page) => handleNavigate(page)}
            />
          )}
          {loading && !hasLoaded ? (
            <PageSkeleton
            ...
            <>
                ...
                <>
                  <Dashboard
                    proposals={proposals.map((proposal) => ({
                      ...proposal,
                      proposal_status:
                        proposal.status === "won" ? "Fechada" : proposal.status,
                      contract_signed: Boolean(proposal.linkedContract?.signed),
                      contract_start_date:
                        proposal.linkedContract?.start_date || null,
                      contract_end_date:
                        proposal.linkedContract?.end_date || null,
                      contract_term: proposal.contractTermMonths
                        ? `${proposal.contractTermMonths} meses`
                        : "",
                      contract_file_url: proposal.linkedContract?.id || null,
                      setup_value: proposal.setupValue,
                      monthly_value: proposal.monthlyValue,
                      responsible_id: proposal.responsibleId,
                      main_service: proposal.mainService,
                    }))}
              ...
              {activePage === "contracts" && dataProvider === "supabase" && (
                <SupabaseContractsPage />
              )}
              ...
              {activePage === "recurring-accounts" && (
                <FinancialPageLayout
                  ...
                  onNavigate={handleNavigate}
                >
                  <RecurringAccountsPage onNavigate={handleNavigate} />
                </FinancialPageLayout>
              )}
              ...
              {activePage === "client-contract-review" && (
                <ClientContractReviewPage />
              )}
              ...
              {[
                ...
                <FinancialSettingsPage section={activePage} />
              )}
              {activePage === "financial-import" && <FinancialImportPage />}
              {["softwares", "providers", "bank-accounts", "cards"].includes(
                activePage,
              ) && (
                <FinancialMasterDataPage
                  section={activePage}
              ...
              {activePage === "intelligence-attention" && (
                <PulseAlertsPage
                  alerts={pulseAlerts}
                  teamMembers={teamMembers}
                  onChanged={refreshPulse}
                  onNavigate={handleNavigate}
                />
              )}
              {activePage === "pulse-alerts" && (
                <PulseAlertsPage
                  alerts={pulseAlerts}
                  teamMembers={teamMembers}
                  onChanged={refreshPulse}
              ...
              {activePage.startsWith("intelligence-") &&
                !["intelligence-attention", "intelligence-today"].includes(
                  ...
                  <MugoIntelligencePage
                    ...
                    loading={loading}
                    error={errorMessage || intelligenceError}
                    section={activePage.replace("intelligence-", "")}
                    onAskAI={() => setAssistantOpen(true)}
                  />
                )}
            </>
