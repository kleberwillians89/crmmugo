...
export function buildInsights(data = {}, now = new Date()) {
  ...
  clients.forEach((client) => {
    ...
    const responsibleId = proposals.find((proposal) => proposal.client_id === client.id)?.responsible_id
    if (!responsibleId) insights.push(makeInsight({ id: `no-owner-${client.id}`, title: 'Cliente sem responsável', description: `${name} não possui responsável comercial identificado.`, severity: 'Média', urgency: 6, confidence: 95, clientId: client.id }))
