...
export function WhatsAppPage({ clients = [], contracts = [], installments = [], proposals = [], onNavigate = () => {}, canWrite = false, isAdmin = false }) {
  ...
  function handleOperationError(cause){setError(cause.status===403?'Sua sessão expirou. Entre novamente no CRM.':cause.code==='UPSTREAM_TIMEOUT'||cause.status===504?'O MugoZap demorou mais que o esperado. Os dados anteriores foram preservados.':cause.message);if(cause.status===403)setConnection('auth-error');else if(cause.code==='UPSTREAM_TIMEOUT'||cause.status===504)setConnection('unstable')}
