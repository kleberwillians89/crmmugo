...
// Subnavegação horizontal da área de Comunicação. Cada item aponta para uma rota
// que já existe (App.jsx) — nada é duplicado aqui.
...
const COMMUNICATION_SECTIONS = new Set(['inbox', 'contacts', 'collections', 'templates', 'automations'])
...
export function WhatsAppPage({ section = 'inbox', page = 'inbox', clients = [], contracts = [], installments = [], proposals = [], onNavigate = () => {}, onAskAI = () => {}, canWrite = false, isAdmin = false }) {
  ...
  const [commercialContext,setCommercialContext]=useState({opportunity:null,owner:null})
  ...
  const refresh = useCallback(async (quiet=false, force=false) => {
    ...
    try {
      ...
      setSelectedId(current=>{const deepLink=new URLSearchParams(window.location.search).get('conversation');if(deepLink&&rows.some(item=>conversationKey(item)===deepLink))return deepLink;return current&&rows.some(item=>conversationKey(item)===current&&hasValidConversationIdentifier(item))?current:''})
      return rows
  ...
  useEffect(()=>{let active=true;const request=selectedId?getConversationCommercialContext(selectedId):Promise.resolve({opportunity:null,owner:null});request.then(data=>{if(active)setCommercialContext(data||{opportunity:null,owner:null})}).catch(()=>{if(active)setCommercialContext({opportunity:null,owner:null})});return()=>{active=false}},[selectedId])
  useEffect(()=>{
  ...
  return <section className={`whatsapp-page section-${section}`}>
    <header className="whatsapp-compact-header"><div className="whatsapp-page-heading"><ProductBreadcrumbs page={page} compact/><div><h1>{presentation.title}</h1>{showChannelStatus&&<span className={`whatsapp-status-badge ${connection}`}>{loading?'Verificando':connection==='connected'?'Conectado':connection==='unstable'?'Instável':connection==='auth-error'?'Indisponível':connection==='initializing'?'Verificando':'Indisponível'}</span>}</div><p>{presentation.description}</p>{showChannelStatus&&<small>Realtime: {realtimeState==='online'?'online':realtimeState==='connecting'?'conectando':realtimeState==='reconnecting'?'reconectando':realtimeState==='fallback'?'modo fallback':'indisponível'}</small>}</div>{section==='inbox'&&<dl><div><dt>Conversas</dt><dd>{summary.conversations_open??conversations.length}</dd></div><div><dt>Aguardando</dt><dd>{summary.waiting_human??conversations.filter(x=>x.awaitingHuman).length}</dd></div><div><dt>Bot ativo</dt><dd>{summary.bot_active??conversations.filter(x=>modeLabel(x)==='Bot ativo').length}</dd></div></dl>}<div className="whatsapp-header-actions">{(section==='inbox'||section==='contacts')&&<button className="button" disabled={!canWrite} title={!canWrite?'Seu perfil possui acesso somente para leitura.':undefined} onClick={()=>setNewContactOpen(true)}><UsersRound size={15}/><span>Novo contato</span></button>}{section==='inbox'&&<button className="button secondary" onClick={()=>onNavigate('collections')}><CircleDollarSign size={15}/><span>Cobrar cliente</span></button>}<button className="button secondary" onClick={()=>refresh(false,true)} disabled={loading}><RefreshCw className={loading?'spin':''} size={15}/><span>{loading?'Atualizando…':'Atualizar'}</span></button></div></header>
    {COMMUNICATION_SECTIONS.has(section)
      ? <nav className="whatsapp-tabs whatsapp-domain-nav" aria-label="Áreas de comunicação">{COMMUNICATION_NAV.map(([id,label])=><button key={id} type="button" aria-current={section===id?'page':undefined} className={section===id?'active':''} onClick={()=>section!==id&&onNavigate(id)}>{label}</button>)}</nav>
    ...
    {loading?<PageSkeleton type="dashboard"/>:tab==='inbox'?<div className={`whatsapp-workspace${selected?' has-selection':''}${client?' has-context':''}`}>
      ...
      <main className="chat-panel">{selected?<><header><div className="chat-contact"><button className="mobile-back" onClick={()=>setSelectedId('')}>Voltar</button><span className="contact-avatar">{selected.name.slice(0,1).toUpperCase()}</span><div><strong>{selected.name}</strong><small>{formatPhoneForDisplay(selectedIdentifier)} · {modeLabel(selected)}{selected.owner?` · ${selected.owner}`:''}</small></div></div><div className="chat-actions"><button onClick={onAskAI} aria-label="Pergunte à Mugô" title="Pergunte à Mugô"><Sparkles size={16}/></button><button className="context-toggle" onClick={()=>setContextOpen(true)}>Contato</button>{canWrite&&<div className="chat-action-menu"><button aria-label="Mais ações" aria-expanded={actionsOpen} onClick={()=>setActionsOpen(current=>!current)}><MoreHorizontal size={17}/></button>{actionsOpen&&<div><button disabled={actionRef.current} onClick={()=>mutate(()=>assignConversation(selected,''),'Conversa assumida.')}>Assumir</button><button disabled={actionRef.current} onClick={()=>mutate(()=>pauseAutomation(selected),'Automação pausada.')}>Pausar bot</button><button disabled={actionRef.current} onClick={()=>mutate(()=>resumeAutomation(selected),'Automação retomada.')}>Retomar bot</button><button disabled={actionRef.current} onClick={()=>mutate(()=>closeConversation(selected),'Conversa encerrada.')}>Encerrar</button><button disabled={actionRef.current} onClick={()=>setLinkModal(true)}><Link2 size={13}/>{client?'Alterar vínculo':'Vincular cliente'}</button>{selectedLink&&<button disabled={actionRef.current} onClick={unlinkClient}>Desvincular</button>}<button onClick={()=>navigator.clipboard.writeText(`+${selectedIdentifier}`)}><Copy size={13}/>Copiar número</button><a className="whatsapp-action-link" href={`https://wa.me/${selectedIdentifier}`} target="_blank" rel="noreferrer"><ExternalLink size={13}/>Abrir WhatsApp</a></div>}</div>}</div></header>
        <div className="chat-management">{isAdmin&&users.length>0&&<><label>Responsável<select value={selected.owner||''} onChange={e=>mutate(()=>assignConversation(selected,e.target.value))}><option value="">Sem responsável</option>{users.map(user=><option key={user.id||user.email} value={user.name||user.email}>{user.name||user.email}</option>)}</select></label>{meta.statuses?.length>0&&<small>Status disponíveis: {meta.statuses.join(', ')}</small>}</>}{canWrite&&<button className="button secondary commercial-handoff-button" disabled={actionRef.current} onClick={commercialHandoff}>Encaminhar para {commercialContext.owner?.name||'comercial'}</button>}</div>
