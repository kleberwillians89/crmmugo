...
export function WhatsAppPage({ clients = [], contracts = [], installments = [], proposals = [], onNavigate = () => {}, canWrite = false, isAdmin = false }) {
  ...
  useEffect(()=>{const conversation=conversations.find(item=>item.waId===selectedId);if(conversation)loadHistory(conversation)},[selectedId,conversations,loadHistory])
  useEffect(()=>{
    if(!selectedId)return
    let active=true,timer
    const poll=async()=>{
      const conversation=conversations.find(item=>item.waId===selectedId)
      if(conversation)await Promise.allSettled([loadHistory(conversation,true),refresh(true,true)])
