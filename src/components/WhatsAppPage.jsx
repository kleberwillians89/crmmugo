...
export function WhatsAppPage({ clients = [], contracts = [], installments = [], proposals = [], onNavigate = () => {}, onAskAI = () => {}, canWrite = false, isAdmin = false }) {
  ...
  const [tab,setTab]=useState('inbox'),[conversations,setConversations]=useState(visualConversations),[selectedId,setSelectedId]=useState(visualMock?conversationKey(visualConversations[0]):''),[messages,setMessages]=useState(visualMock?whatsappVisualMessages:[])
  ...
  async function openCollection(item){
    ...
    try{if(isTemplateSyncStale()){const result=await refreshTemplateStatuses(),template=result.templates.find(row=>row.name==='mugo_alerta_pagamento_pendente');if(template)setTemplateStatus(template)}const conversation=await findConversationByPhone(phone);if(conversation){await refresh(true);setSelectedId(conversationKey(conversation));setTab('inbox')}else{setStartModal(true);setError('Nenhuma conversa anterior encontrada. Você pode iniciar uma nova conversa.')}}catch(cause){setError(cause.message)}
  }
