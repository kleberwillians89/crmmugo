...
export function WhatsAppTemplateSendModal({ template, clients = [], onClose, onSend, dryRun = true }) {
  ...
  const [recipient,setRecipient]=useState(''),[clientId,setClientId]=useState(''),[values,setValues]=useState({}),[sending,setSending]=useState(false),[error,setError]=useState(''),[validation,setValidation]=useState(null),[sendState,setSendState]=useState(''),[ambiguousFingerprint,setAmbiguousFingerprint]=useState('')
