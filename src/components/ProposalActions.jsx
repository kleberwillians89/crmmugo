...
export function ProposalActions({proposal,onEdit,onDuplicate,onConvert,onLink,onArchive}){return <details className="proposal-actions" onClick={(event)=>event.stopPropagation()}><summary aria-label={`Ações de ${proposal.clientName||'proposta'}`}><MoreHorizontal size={18}/></summary><div className="proposal-actions-menu">
  ...
  <button type="button" disabled={proposal.hasContract} onClick={()=>onLink?.(proposal)}><Link2 size={14}/>Vincular a contrato existente</button>
